...
public class MultiplicationOperatorTransposeOptimized extends Matrix {
    private final ComputeNode left;
    private final ComputeNode right;
    public final String operatorName;

    public MultiplicationOperatorTransposeOptimized(ComputeNode left, ComputeNode right,
                                                    String operatorName) {
    ...
    public void computeResult() {
        ...
        final float[] slice_one = new float[TILE_SIZE*TILE_WIDTH];
        final float[] slice_two = new float[TILE_SIZE*TILE_WIDTH];
        ...
        for (int oRow = 0; oRow < oRows; oRow+=TILE_SIZE) {
            for (int oCol = 0; oCol < oCols; oCol+=TILE_SIZE) {
                ...
                int tileHeight = Math.min((oRows - oRow), TILE_SIZE);
                ...
                for (int cVal = 0; cVal < commonDim; cVal+=TILE_WIDTH) {
                    ...
                    for (int sliceRow = 0; sliceRow < tileHeight; sliceRow++) {
                        ...
                        for (int sliceCol = 0; sliceCol < tileWidth; sliceCol++) {
                            ...
                            oBak[(sliceRow+oRow) * oCols + (sliceCol+oCol)] += accumulator;
                        }
                    }
                }
            }
